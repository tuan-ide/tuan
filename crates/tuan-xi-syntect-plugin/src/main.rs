...
use xi_plugin_lib::{Cache, Error, Plugin, StateCache, View, mainloop};
use xi_rope::{DeltaBuilder, Interval, Rope, RopeDelta, RopeInfo};
use xi_trace::{trace, trace_block};

use syntect::dumps::from_binary;
use syntect::parsing::{
    ParseState, SCOPE_REPO, ScopeRepository, ScopeStack, ScopedMetadata, SyntaxSet,
};
...
impl<'a> PluginState {
    ...
    fn compute_syntax(
        ...
        for (cursor, batch) in ops {
            ...
            prev_cursor = cursor;
            scope_state.apply(&batch).unwrap();
        }
